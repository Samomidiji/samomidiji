(function($) {
	...
	$(document).on('submit','#commentform',function(e){
		...
		$.ajax({
			type : 'POST',
			url : roph_ajax_comment_params.ajaxurl, // admin-ajax.php URL
			...
			success: function ( response ) {
                if (response.status_or_comment_html == 1) {
					...
					if ( commentlist.length > 0 ){
						$(".comments-title").text(response.comments_number_text);
						// if in reply to another comment
						if ( respond.parent().hasClass( 'comment-li' ) ){
						...
						// if no comments yet
						comment_html = '<h3 class="comments-title">' + response.comments_number_text + '</h3><ul class="comment-list">' + response.status_or_comment_html + '</ul>';
